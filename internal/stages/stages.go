...
// GetDefinition returns the TesterDefinition for the tinygpt course.
func GetDefinition() tester_definition.TesterDefinition {
	return tester_definition.TesterDefinition{
		TestCases: []tester_definition.TestCase{
			// Phase 1: Transformer
			e01TokenizationTestCase(),
			e02EmbeddingsTestCase(),
			e03AttentionTestCase(),
			e04TransformerBlockTestCase(),
			e05GptAndGenerateTestCase(),
			// Phase 2: Optimization
			e06QuantizationAndKVCacheTestCase(),
			e07ProfilingAndCompressionTestCase(),
		},
	}
}
...
func javaRule(testDriver string) tester_definition.LanguageRule {
	return tester_definition.LanguageRule{
		DetectFile: "src/main/java/dev/tensorhero/tinygpt/Tokenizer.java",
		Language:   "java",
		Source:     "src/main/java/dev/tensorhero/tinygpt/Tokenizer.java",
		Flags: []string{
			"-encoding", "UTF-8",
			// tinynum (read-only dependency)
			...
			// tinytorch (read-only dependency — full solution)
			"src/main/java/dev/tensorhero/tinytorch/Tensor.java",
			...
			// tinygpt (student-implemented)
			"src/main/java/dev/tensorhero/tinygpt/Tokenizer.java",
			"src/main/java/dev/tensorhero/tinygpt/Embedding.java",
			"src/main/java/dev/tensorhero/tinygpt/EmbeddingBackward.java",
			"src/main/java/dev/tensorhero/tinygpt/PositionalEncoding.java",
			"src/main/java/dev/tensorhero/tinygpt/Attention.java",
			"src/main/java/dev/tensorhero/tinygpt/TransformerBlock.java",
			"src/main/java/dev/tensorhero/tinygpt/GPT.java",
			"src/main/java/dev/tensorhero/tinygpt/Quantizer.java",
			"src/main/java/dev/tensorhero/tinygpt/KVCache.java",
			"src/main/java/dev/tensorhero/tinygpt/Profiler.java",
			"src/main/java/dev/tensorhero/tinygpt/Pruner.java",
...
func pythonRule(testDriver string) tester_definition.LanguageRule {
	return tester_definition.LanguageRule{
		DetectFile: "tinygpt/__init__.py",
		Language:   "python",
		Source:     "tinygpt/__init__.py",
		RunCmd:     "python3",
